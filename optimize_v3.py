...
# Normalize content to LF to make multiline replacements simple
...
old_js_bind_lf = old_js_bind.replace("\r\n", "\n")
new_js_bind_lf = new_js_bind.replace("\r\n", "\n")

if old_js_bind_lf in content_lf:
    content_lf = content_lf.replace(old_js_bind_lf, new_js_bind_lf)
...
old_sidebar_marker_lf = old_sidebar_marker.replace("\r\n", "\n")
...
if old_sidebar_marker_lf in content_lf:
    content_lf = content_lf.replace(old_sidebar_marker_lf, fragments_definitions)
    print("Inserted fragments definitions successfully.")
else:
    print("Could not find old_sidebar_marker!")

# 3. Replace the Telegram expander + alert block (original L.920 to 1019)
old_telegram_alert_block = """    with st.expander("🚨 텔레그램 알림 설정"):
...
new_telegram_alert_block = """    with st.expander("🚨 텔레그램 알림 설정"):
        st.subheader("🔑 텔레그램 연동 설정")
...
old_telegram_alert_block_lf = old_telegram_alert_block.replace("\r\n", "\n")

if old_telegram_alert_block_lf in content_lf:
    content_lf = content_lf.replace(old_telegram_alert_block_lf, new_telegram_alert_block)
    print("Replaced Telegram + Alert block successfully.")
else:
    print("Could not find old_telegram_alert_block!")
...
old_right_col_lf = old_right_col.replace("\r\n", "\n")

if old_right_col_lf in content_lf:
    content_lf = content_lf.replace(old_right_col_lf, new_right_col)
    print("Replaced Virtual Trading Panel successfully.")
else:
...
with open(file_path, "w", encoding="utf-8") as f:
    f.write(content_lf)
print("Finished all optimizations in optimize_v3.py!")
